        ...
        return(
           <nav className="fixed top-0 left-0 w-full z-50">
            ...
            <div className="hidden md:flex space-x-8">
              {LINKS.map((link,index)=>(
                <a href={link.href} key={index} className="text-white hover:text-pink-400 hover:underline hover:decoration-2 transition duration-300">
                    {link.label}
                </a>
              )
              )}
           ...
           <div className="md:hidden">
            <button onClick={()=> setMenuOpen(!menuOpen)} className="text-white focus:outline-none" aria-label={menuOpen ? "Close menu" : "Open menu"}>
              ...
              {menuOpen &&(
                  <div className="md:hidden p-2 bg-stone-950/30 backdrop-blur-lg rounded-xl flex-col space-y-4 max-w-6xl mx-auto">
                    {LINKS.map((link, index)=>(
